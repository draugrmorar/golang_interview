# Память в Go: Стек, Куча и Garbage Collector

Обычно в языках программирования есть две области памяти — **Stack** и **Heap**:

- На **Stack** (грубо говоря, это стек нашего метода) мы храним переменные, вызовы, указатели на ссылочные типы.
- В **Heap** обычно хранится то, что называется ссылочными (reference) объектами — всё, что не примитив.
- **String** обычно хранится на Heap почти во всех языках, а на Stack лежат boolean, int и указатели на эти референсные типы.

Почему такие названия — Stack и Heap? Потому что **Stack** — это стек. А **Heap** — это когда их в кучу навалили и каждую ищите по примерному адресу — надо искать смещение, коллектить и прочее.
...
**Плюсы:**
- Экономия памяти: каждая горутина начинала с небольшого стека.
...
- Горутина стартует с маленького стека (~2KB).
...
- Когда стек переполняется:
  1. Go выделяет новый, более крупный участок памяти (обычно в стеке, но иногда в куче)
...
**Почему LIFO?**  
Если бы стек работал не по LIFO, то освобождать память пришлось бы вручную, а это усложнило бы управление ресурсами. LIFO позволяет разом освобождать всю память функции без дополнительных операций.

## Минимальный размер стека Go

```
_StackMin = 2048 // стек горутин изначально весит всего 2 КБ
```

Горутины могут автоматически увеличиваться/уменьшаться в соответствии с нашими потребностями. Размер стека изменялся из-за стратегии его аллокации:
- Go 1.2: стек горутины был увеличен с 4 КБ до 8 КБ.
- Go 1.4: стек горутины уменьшился с 8 КБ до 2 КБ.

**Команда по которой можно посмотреть как Go аллоцирует память:**
```bash
...
GC в Go использует **трехцветный алгоритм**, который работает одновременно с программой (конкурентно) и по частям (инкрементально).
...
STW происходит на каждой сборке мусора, но очень коротко. Цикл GC состоит из фаз:
- Sweep termination (STW) — подчистка хвостов от предыдущего цикла.
- Mark setup (STW) — включение write barrier, подготовка к маркировке.
- Marking (конкурентная) — основная работа, триколор.
- Mark termination (STW) — выключение write barrier, финальная проверка.
- Sweep (конкурентная) — освобождение белых объектов.

Setup и termination — это начало и конец фазы маркировки (mark phase). Именно эти две паузы STW составляют основное время, когда мир «стоит». Современный Go укладывает их в десятки-сотни микросекунд.
...
Процессор читает память не побайтово, а блоками (обычно по 4 или 8 байт). Если данные не выровнены по границам этих блоков, процессору приходится делать два чтения вместо одного.

- **Производительность:** Выровненный доступ в 2-3 раза быстрее.
- **Атомарность:** Некоторые операции (atomic) требуют выравнивания.
- **Кроссплатформенность:** На некоторых архитектурах невыровненный доступ вызывает panic, иначе программа может упасть на 32-битных системах.
- **Память:** Из-за неправильного порядка полей структура может занимать в 2 раза больше места.

У каждого типа данных есть свои требования к выравниванию:
- **Маленькие типы** (bool, byte) могут начинаться где угодно.
- **Типы побольше** (int16, int32) требуют четных адресов.
- **Большие типы** (int64, float64, указатели) должны начинаться с адреса, кратного 8.

Когда вы создаете структуру с разными полями, компилятор автоматически добавляет "пустоты" (padding) между полями, чтобы соблюсти эти требования.

## Главный практический вывод

Если вы пишете структуру с разными полями, расположите поля **от самого большого к самому маленькому**. Это минимизирует "пустоты" между ними. Особенно важно для структур, которые создаются в больших количествах.

На 64-битных системах современные процессоры обычно справляются с невыровненным доступом без падений (только медленнее), но на 32-битных системах и некоторых архитектурах (ARM) это может вызвать аварийное завершение программы.

---

## GC Pacing 
— это алгоритм, который регулирует частоту и агрессивность сборок, чтобы балансировать между использованием CPU и памятью.

**Цель:** не дать куче расти бесконечно (OOM), но и не тратить 50% CPU на GC.

Как работает:
- GC следит за скоростью выделения памяти приложением (allocation rate).
- Если приложение выделяет память быстрее, чем GC успевает её освобождать, GC начинает работать чаще и агрессивнее.
- Если выделение медленное — GC расслабляется.
- Основной параметр — GOGC (по умолчанию 100): если куча выросла на 100% с прошлой сборки — запускаем новую.


> Источник: [https://habr.com/ru/articles/923398/](https://habr.com/ru/articles/923398/)