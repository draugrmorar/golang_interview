...
### Типы Context (несколько базовых типов контекстов)
- **Background Context** — корневой контекст, который никогда не отменяется
- **WithCancel** — контекст с ручной отменой
- **WithTimeout/WithDeadline** — контекст с автоматической отменой по времени
- **WithValue** — контекст для передачи данных
...
- **Проверка отмены:** В цикле мы регулярно проверяем `ctx.Done()` через select, что позволяет немедленно прервать работу при получении сигнала отмены.
- **Graceful shutdown:** Функция не просто завершается, а возвращает осмысленную ошибку через `ctx.Err()`, что позволяет вызывающему коду понять причину остановки.
- **Неблокирующая проверка:** Конструкция select с default позволяет проверить отмену без блокировки выполнения.
...
### Важные аспекты временного управления

- **Превентивная отмена:** Context автоматически отменяется при превышении лимита времени, предотвращая зависание операций.
- **Каскадное наследование:** Дочерние контексты наследуют ограничения родительских, но могут устанавливать более строгие лимиты.
- **Детализированный контроль:** WithDeadline позволяет синхронизировать операции с внешними событиями (например, окончанием рабочего дня или закрытием торговой сессии).
...
- Используйте типизированные ключи для избежания коллизий
- Храните только immutable данные
- Не передавайте через Context обязательные параметры функций
- Используйте для метаданных: ID запросов, токены, настройки трассировки
...
- **Типизированные ключи:** Использование `contextKey` типа предотвращает конфликты между разными частями приложения.
- **Helper функции:** GetUserID, SetUserID инкапсулируют логику работы с контекстом и обеспечивают type safety.
- **Цепочка обработки:** Данные из контекста доступны во всей цепочке вызовов функций без изменения их сигнатур.
- **Разделение ответственности:** Каждая функция извлекает только те данные, которые ей нужны.
...
В HTTP обработчиках Context особенно важн для:
...
1. **Централизованная отмена:** Один вызов cancel() останавливает все горутины
2. **Graceful shutdown:** Горутины завершают текущую работу и корректно выходят
3. **Timeout для группы операций:**
   ...
   ```
4. **Каскадная отмена:** Дочерние контексты автоматически отменяются при отмене родительского
...
1. **Автоматическая отмена запросов:** При отмене контекста HTTP запрос немедленно прерывается, освобождая сетевые ресурсы.
...
**Graceful Shutdown** — это механизм корректного завершения программы, когда она успевает:
- Завершить текущие операции
- Закрыть соединения
- Сохранить данные
- Освободить ресурсы

Альтернатива: жесткое завершение (kill -9), когда программа просто убивается "на полуслове".

### Ключевые элементы:
1. Перехват сигналов (SIGINT, SIGTERM)
2. Контекст с таймаутом — сколько ждать завершения операций
3. Блокировка новых операций (сервер перестает принимать запросы)
4. Ожидание завершения текущих
...
- Любые программы с долгими операциями

---

> Источник: [https://gopedia.ru/study/advanced/context-and-cancellation/](https://gopedia.ru/study/advanced/context-and-cancellation/)
